...
const onSelectableChange=({selectedRows})=>{
  ...
  const dataUngraduate=useMemo(()=>dataSiswa.data.filter(item=>item.student_status<2),[dataSiswa.data])
  const dataGraduate=useMemo(()=>dataSiswa.data.filter(item=>item.student_status==2),[dataSiswa.data])
  ...
  return (
    <>
      ...
      <div className="page-content">
        <h3>
          Kelulusan <span style={{ fontSize: "0.8em", color: "gray" }}>List</span>
        </h3>
    ...
    <Row  className="mt-4">
     ...
     <Button className="d-flex justify-content-around align-items-center" color="primary"><MdNavigateNext color="white" size={30}/>Proses Lulus</Button>
     <Button   className="d-flex justify-content-around align-items-center" color="danger"><MdNavigateBefore  color="white" size={30}/>Batal Lulus</Button>
      </div>
