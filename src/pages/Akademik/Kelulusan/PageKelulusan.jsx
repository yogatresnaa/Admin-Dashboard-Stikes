...
function PageKelulusan() {
    ...
    const onSubmitStatusChangeHandler = useCallback(
        async (e, type) => {
            ...
            const updatedData = dataSiswa.data
                .filter((item) =>
                    selectedItems.some(
                        (itemCompare) =>
                            itemCompare.student_id == item.student_id
                    )
                )
                .map((item) => {
                    return {
            ...
            const unUpdatedData = dataSiswa.data.filter(
                (item) =>
                    !updatedData.some(
                        (itemCompare) =>
                            item.student_id == itemCompare.student_id
                    )
            )

            setDataSiswa((prevState) => ({
                ...prevState,
                data: [...updatedData, ...unUpdatedData],
            }))
            setSelectedItems([])
